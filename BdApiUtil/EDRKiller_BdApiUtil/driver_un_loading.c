...
BOOL WriteDriverToFile(IN LPCWSTR pszDriverName, IN PBYTE pbDriver, IN DWORD dwDriverSize, OUT LPWSTR* pszFullDriverPath) {
    ...
    infoW_t(L"Generated driver directory \"%ls\"", szDriverPath);
    ...
    infoW_t(L"WriteFileW - Written driver to \"%ls\"", szDriverPath);
...
BOOL LoadDriver(IN LPCWSTR lpwcDriverName, IN LPCWSTR lpwcDriverPath) {
    ...
    if (!StartServiceW(
        ...
        info_t("StartServiceW - Driver started at 0x%p", hService);
    }
...
HANDLE GetDeviceHandle(IN LPCWSTR lpwcDriverSymlink) {
    ...
    hDevice = CreateFileW(
        lpwcDriverSymlink,			// Symbolic link to driver
